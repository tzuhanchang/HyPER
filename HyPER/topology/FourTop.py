...
def FourTop_allhad(HyPER_outputs: str | pd.DataFrame):
    ...
    for i in tqdm(range(len(results)), desc="Reconstructing", unit='event'):
        ...
        # We need 4 top quarks
        while completed_patterns < 4:
        ...
        HyPER_best_top3.append(selected_HE[2])
        HyPER_best_top4.append(selected_HE[3])
        HyPER_best_w1.append(selected_GE[0])
        HyPER_best_w2.append(selected_GE[1])
        HyPER_best_w3.append(selected_GE[2])
        HyPER_best_w4.append(selected_GE[3])

    results['HyPER_best_top1'] = HyPER_best_top1
    results['HyPER_best_top2'] = HyPER_best_top2
    results['HyPER_best_top3'] = HyPER_best_top3
    results['HyPER_best_top4'] = HyPER_best_top4
    results['HyPER_best_w1'] = HyPER_best_w1
    results['HyPER_best_w2'] = HyPER_best_w2
    results['HyPER_best_w3'] = HyPER_best_w3
    results['HyPER_best_w4'] = HyPER_best_w4
